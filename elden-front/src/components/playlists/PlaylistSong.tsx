...
export default function PlaylistSong({
  ...
  async function deleteFromPlaylist() {
    try {
      const res = await requestRemoveSongToPlaylist(idPlaylist, idSong);
      if (res.status === 200) {
        onDelete();
        dispatch(showSuccess(`Song removed from playlist`));
      } else {
        dispatch(showError(`There was an error, try again`));
      }
    } catch (error) {
      ...
      <div className="user-song">
        ...
        <div className="user-song__info-cont">
          ...
          <div className="user-song__ctl-cont">
            ...
            <IconButton aria-label="play" size="large" onClick={playSong}>
